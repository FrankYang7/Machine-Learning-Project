...
def computeE(centered_X):
    ...
    # please note the output vals and corresponding vectors are in ascending order, and the shape of vectors are (d, p)
    # (3) reverse the order of eig_val and eig_vec to make it in descending order
    X_cov = np.cov(centered_X.T)
    eig_val, eig_vec = np.linalg.eigh(X_cov)
    copy_val = np.zeros(len(eig_val))
    copy_vec = np.zeros([len(eig_vec), eig_vec.shape[1]])
    for i in range(len(eig_val)):
        copy_val[i] = eig_val[i]
    for i in range(len(eig_vec)):
        for j in range(len(eig_vec[i])):
            copy_vec[i][j] = eig_vec[i][j]
    for i in range(len(eig_val)):
        eig_val[i] = copy_val[len(eig_val)-1-i]
    for i in range(len(eig_vec)):
        for j in range(len(eig_vec[i])):
            eig_vec[i][j] = copy_vec[i][len(eig_vec[i])-1-j]
