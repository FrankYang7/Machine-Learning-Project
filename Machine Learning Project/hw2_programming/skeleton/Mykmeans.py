...
class Kmeans:
    def __init__(self,k=8): 
        ...
        self.center = None    # Placeholders for the position of all k centers
    ...
    def fit(self, X, y):
        ...
        while not is_converged:
            ...
            # during iteration, for each data assessed, you can add the point position to corresponding center, if you assign x[0],x[1] to cluster 0, new_center['center'] = [[1+2], [0]]
    ...
    def predict(self,X):
        ...
        for i in range(len(X)):
            dis = computeDis(X[i], self.center)
            prediction[i] = self.cluster_label[assignCen(dis)]
        return prediction
...
def computeDis(x, centers):
    # placeholders of distances to all centers, can be ignored or removed
...
# input:
#   new_center:  dict, structre specified in previous comments line 38-41
# Output
#   centers:   k center position, (k, d)
def updateCen(new_center):
    ...
    for i in range(len(new_center['center'])):
        centers[i] = new_center['center'][i]/new_center['num_sample'][i]
...
def computeError(X, assign, centers):
    ...
    for i in range(len(X)):
        for j in range(X.shape[1]):
            sub = X[i][j] - centers[assign[i]][j]
            error = error + pow(sub, 2)
