...
# output: results of pluging the value into (e^-x-e^x)/(e^-x+e^x), you can use np.exp()
...
def tanh(x):
    ...
    for i in range(n):
        for j in range(d):
            x_cur = x[i][j];
            out[i][j] = (np.exp(x_cur)-np.exp(-x_cur))/(np.exp(x_cur)+np.exp(-x_cur))
    return out 
...
def softmax(x):
    ...
    for i in range(n):
        current_sum = 0
        for j in x[i]:
            current_sum = current_sum + np.exp(j)
        for p in range(d):
            out[i][p] = np.exp(x[i][p])/current_sum
    return out
...
class MLP:
    ...
    def forward(self, x):
        # placeholders
        y = np.zeros([len(x), 10])
        z = tanh(np.dot(x,self.w)+self.w0)
        y = softmax(np.dot(z,self.v)+self.v0)
        return z, y
    ...
    # Input: z, output of the intermediate layer (1000, num_hid)
    #        y, output of the last layer (1000, 10)
    #        x, input of first layer (1000, 64)
    #        r, gt one-hot labels (1000, 10)
    #        v, (4, 10)
    # Output: out, gra_w, (64, num_hid)
    def dEdw(self, z, y, x, r):
        # placeholder
        output1 = (y-r)@self.v.T
        scater_z = 1-np.power(z,2)
        out = x.T@(output1*scater_z)
        return out
    ...
    def dEdw0(self, z, y, r):
        ...
        out = ((y-r)@self.v.T*(1-np.power(z,2))).sum(axis=0)
        return out
