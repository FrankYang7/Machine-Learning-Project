...
class Decision_tree:
    ...
    def generate_tree(self,data,label):
        ...
        if node_entropy < self.min_entropy:
            # ------------------- Add y
            # our lines here --------------------
            count_list = np.zeros(max(label)+1)
            for i in range(len(label)):
                count_list[label[i]] = count_list[label[i]]+1
            cur_node.label = np.argmax(count_list)
            return cur_node
    ...
    def select_feature(self,data,label):
        ...
        for i in range(len(data[0])):
            ...
            # -------------- Add your line here -------------------------------
            combined_entropy = self.combined_entropy(left_y,right_y)
            # select the feature with minimum entropy (set best_feat)
            # -------------- Add your line here -------------------------------
            if i == 0:
                min_entropy = combined_entropy
            elif combined_entropy < min_entropy:
                min_entropy = combined_entropy
                best_feat = i
        return best_feat
    ...
    def combined_entropy(self,left_y,right_y):
        ...
        weights = [len(left_y)/(len(left_y)+len(right_y)), len(right_y)/(len(left_y)+len(right_y))]
        result = self.metric(label=left_y)*weights[0]+self.metric(label=right_y)*weights[1]
    ...
    def entropy(self,label):
        ...
        # -------------- Add your line here -------------------------------
        if len(label) == 0:
            node_entropy =  -np.sum((np.log2(1e-15)))
        else:
            count_list = np.zeros(max(label) + 1)
            for i in range(len(label)):
                count_list[label[i]] = count_list[label[i]] + 1
            count = count_list/np.sum(count_list)
            node_entropy = -np.sum((count * np.log2(count+1e-15)))
        return node_entropy
    ...
    def gini_index(self, label):
        ...
        if len(label) == 0:
            ...
            for i in range(len(label)):
                count_list[label[i]] = count_list[label[i]] + 1
            count = count_list/np.sum(count_list)
            gini_index = 1 - np.sum(np.power(count,2))
        return gini_index
